...
export function DigitalMenuDropdown() {
  return (
    <div className="absolute left-0 mt-2 w-64 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5">
      <ul className="py-2">
        {services.map((s) => (
          <li key={s.name} className="px-4 py-2 hover:bg-gray-100 flex items-center">
            <span className="mr-2 text-gray-600">{s.icon}</span>
            <div className="flex-1">
              <a href={s.href} target="_blank" rel="noopener noreferrer" className="block text-sm font-medium text-gray-800">
                {s.name}
              </a>
